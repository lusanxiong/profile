...
public class GB2Alpha {
     ...
     * 否则到SpellMap中查??如果没有找到拼音,则返回null,如果找到则返回拼??  
     */  
    private static String getSpellByAscii(int ascii) {   
        if (ascii > 0 && ascii < 160) { // 单字??-英文或半角字??  
    ...
    public static String getFullSpell(String cnStr) {   
        ...
        for (int i = 0, Len = chars.length; i < Len; i++) {   
            int ascii = getCnAscii(chars[i]);//得到单个中文的Ascii??  
            if (ascii == 0) { // 取ascii时出??  
     ...
     * 取中文字符的首字符的英文拼音，并组装
     * 生产bug
     * add by chenjiajie 2009-08-27
     * @param chnStr
     * @return
     */
    public static String connectFirstSpell(String chnStr){
        String resultStr = "";
        if(!StringUtil.isValidStr(chnStr)){
            return null;
        }
        String chn = chnStr.replaceAll("[A-Za-z-]", "");
        String firstStr = getFirstSpell(chn);
        resultStr = firstStr.substring(0,1) + chn;
        return resultStr;
    }
	...
	public static void main(String[] args) {
		System.out.println(GB2Alpha.getFullSpell("刘德华"));   
        //输出结果为：liudehua  
        System.out.println(GB2Alpha.connectFirstSpell("中国香港"));   
        //输出结果为：Z中国香港   
