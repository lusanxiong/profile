...
//		szTmp = Tools.replace(szTmp, " or ", ";");
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (keyStr.indexOf(" " + key) > -1
					|| keyStr.indexOf(key + " ") > -1)
				continue;
			if (i == 0)
				keyStr += key;
			else {
				if (i == 1 && keyStr.indexOf(key) > -1)
					continue;
				keyStr += " and " + key;
			}
	 ...
	 * @return
	 */
	public static String FormattoKeyStr(String keyStr, String s) {

		String orStr = "";
		String andStr = "";
		keyStr = toKeyStr(keyStr);
		keyStr = replace(keyStr, " and ", ",");
		keyStr = replace(keyStr, " or ", ";");
		List or = splitToList(keyStr, ";");
		for (int i = 0; i < or.size(); i++) {
			orStr = (String) or.get(i);
			List and = splitToList(orStr, ",");
			for (int j = 0; j < and.size(); j++) {
				andStr = (String) and.get(j);
				s = replace(s, andStr, "<font class=keystr>" + andStr
						+ "</font>");
			}
		}
		return s;
	...
	public static final String getStringWithLength(String str, int length, String suffix){
	...
	public static void main(String args[]) {
		...
		String ss = StrUtils.getStringWithLength("123",5,"...");
